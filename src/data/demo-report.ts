...
export const STEPS: Step[] = [
  ...
  // 03 비즈니스 모델 (paid)
  {
    ...
    lockedTeaser: "AI가 다시 설계한 BM 5개 항목과 변경 사유가 잠겨 있습니다.",
    blocks: [
      {
        kind: "beforeAfter",
        rows: [
          { field: "문제", before: "부모님 안부가 늘 걱정된다", after: "비동거 자녀가 MCI 부모의 상태 변화를 매일 파악하기 어렵고, 직접 전화는 지속·기록·추세화가 안 된다", why: "'걱정'이라는 감정은 살 수 없습니다. '지속성·기록·추세 감지의 부재'라는 측정 가능한 결핍으로 쪼개야 인터뷰로 통증을 검증할 수 있습니다." },
          { field: "고객", before: "노인, 가족", after: "1차: 비동거 40~60대 자녀 / 2차: 요양·복지기관 / 3차: 통신사", why: "'가족'은 너무 넓습니다. 실제 지불자(자녀)를 1차로 못 박고 대량 유통 채널을 2·3차로 분리해야 메시지·가격·기능이 한 방향으로 정렬됩니다." },
          { field: "솔루션", before: "AI가 전화해 주는 서비스", after: "보호자 음성 AI 케어콜 → 3항목(복약·식사·컨디션) 확인 → 대화 분석 → 보호자 앱 기록·이상 알림", why: "'전화'만으로는 가치가 모호합니다. 확인→기록→알림을 하나의 반복 루프로 고정하면 제품 범위와 원가·품질 기준이 동시에 잡힙니다." },
          { field: "수익모델", before: "월 구독", after: "B2C 월 구독 + B2B 좌석제(기관) + 통신사 번들 제휴", why: "통화가 변동비라 단일 B2C는 원가 충격에 취약합니다. 계약 규모가 큰 B2B·제휴로 원가를 감당하고 성장 경로를 다변화하되, 순서는 B2C 검증이 먼저입니다." },
          { field: "차별점", before: "AI가 대신 전화", after: "익숙한 보호자 목소리 + 임상 자문 설계 + 축적되는 통화·신뢰 데이터", why: "'AI 전화'는 이미 여럿이 합니다. 복제 난도 높은 세 가지로 재정의해야 후발주자·빅플레이어의 모방을 견디는 해자가 됩니다." },
        ],
      },
    ],
    sources: PENDING_SOURCES,
